...
This uses the khmer script ‘split-paired-reads’ to break the reads into left (/1) and right (/2). (This takes a long time! 5m reads is a lot of data...)
We’ll also need to get some Illumina adapter information
::
...
**TIME TO TEST YOUR KNOWLEDGE**: Look at reads
...
BONUS!!! Assuming the E. coli genome is 4.5 Mbp, what is the depth of coverage?
...
* Remove adapters (ILLUMINACLIP:../reads/illuminaClipping.fa:2:30:10)
...
Go check it out on your local computer you’re looking for files named <filename>_fastqc, for example ‘s1_pe_fastqc_report.html’.

    scp -r your_username@IP_address:/Volumes/Pegasus/your_home_folder/Assembly/fastqc .

you’re looking for ‘s1_pe_fastqc_report.html’ and ‘s2_pe_fastqc_report.html’.
